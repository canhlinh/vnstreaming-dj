package com.vnstreaming.ffmpeg.builder;

import com.google.common.base.Preconditions;
import com.google.common.base.Strings;
import com.google.common.collect.ImmutableList;
import com.vnstreaming.ffmpeg.modelmapper.Mapper;
import com.vnstreaming.ffmpeg.options.AudioEncodingOptions;
import com.vnstreaming.ffmpeg.options.EncodingOptions;
import com.vnstreaming.ffmpeg.options.VideoEncodingOptions;
import com.vnstreaming.ffmpeg.probe.FFmpegProbeResult;
import com.vnstreaming.ffmpeg.options.MainEncodingOptions;
...
public class FFmpegOutputBuilder implements Cloneable {
	...
	public FFmpegOutputBuilder setPixelFormat(String value) {
	...
	protected List<String> build(int pass) {
		...
		if (video_enabled) {
			...
			if (video_frame_rate != null) {
				...
				int gop_size = video_frame_rate.intValue() * 2;
				args.add("-g").add(String.valueOf(gop_size));
				args.add("-keyint_min").add(String.valueOf(gop_size));
			}
			...
			if (!Strings.isNullOrEmpty(video_preset)) {
				args.add("-preset").add(video_preset);
			}

			if (!Strings.isNullOrEmpty(video_filter)) {
				args.add("-vf").add(video_filter);
			}
			if(!Strings.isNullOrEmpty(pixel_format)) {
				args.add("-pix_fmt").add(pixel_format);
			}
			args.add("-shortest");
